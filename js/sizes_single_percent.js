...
function rssac002_update_chart(){
  ...
  if(time_interval == 'day'){
    ...
    options.title.text = 'Top ' + num_ranges + ' ' + metric + ' per-day (%)';
    ...
    options.title.text = 'Top ' + num_ranges + ' ' + metric + ' by-week (%) (daily average)';
      ...
      $.each(all_data, function(date, ranges){
        for(ii = 0; ii < series_ranges.length; ii++){
          if(series_ranges[ii].name in all_data[date]){
            series_ranges[ii].data.push(all_data[date][series_ranges[ii].name] / date_totals[date] * 100);
          }else{
